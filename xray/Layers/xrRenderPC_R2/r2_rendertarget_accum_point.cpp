...
void CRenderTarget::accum_point		(light* L)
{
	...
	if (L->flags.bHudMode)
	{
		...
		Device.mProject.build_projection( deg2rad( psHUD_FOV < 1.f ? psHUD_FOV*Device.fFOV : psHUD_FOV /* *Device.fASPECT*/ ), Device.fASPECT, HUD_VIEWPORT_NEAR, g_pGamePersistent->Environment().CurrentEnv->far_plane );
