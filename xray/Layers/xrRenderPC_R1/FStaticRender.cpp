...
void CRender::Calculate				()
{
	...
	if (rmPortals) 
	{
		...
		Portals_xrc.box_options	(CDB::OPT_FULL_TEST);
		Portals_xrc.box_query	(rmPortals,Device.vCameraPosition,box_radius);
		for (int K=0; K<Portals_xrc.r_count(); K++)
		{
			CPortal*	pPortal		= (CPortal*) Portals[rmPortals->get_tris()[Portals_xrc.r_begin()[K].id].index];
