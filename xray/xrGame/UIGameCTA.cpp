...
void CUIGameCTA::SetClGame(game_cl_GameState* g)
{
	...
	m_game = dynamic_cast<game_cl_CaptureTheArtefact*>(g);
	VERIFY(m_game);
...
{
	CUIMpTradeWnd* buy_menu = dynamic_cast<CUIMpTradeWnd*>(m_pCurBuyMenu);
	R_ASSERT(buy_menu);
...
void CUIGameCTA::TryToDefuseAllWeapons	(aditional_ammo_t & dest_ammo)
{
	...
	CActor* actor = dynamic_cast<CActor*> (Level().Objects.net_Find(ps->GameID));
	...
	{
		CWeapon* tmp_weapon = dynamic_cast<CWeapon*>(*i);
		if (tmp_weapon)
...
struct AmmoSearcherPredicate
{
	...
	{
		CWeaponAmmo* temp_ammo = dynamic_cast<CWeaponAmmo*>(item);
		if (!temp_ammo)
			return false;
...
void CUIGameCTA::TryToDefuseGrenadeLauncher(CWeaponMagazinedWGrenade const * weapon, TIItemContainer const & all_items, aditional_ammo_t & dest_ammo)
{
	...
	CWeaponAmmo* temp_ammo = dynamic_cast<CWeaponAmmo*>(*temp_iter);
...
void CUIGameCTA::TryToDefuseWeapon(CWeapon const * weapon, TIItemContainer const & all_items, aditional_ammo_t & dest_ammo)
{
	...
	if (weapon->IsGrenadeLauncherAttached())
		TryToDefuseGrenadeLauncher(dynamic_cast<CWeaponMagazinedWGrenade const *>(weapon), all_items, dest_ammo);
...
void CUIGameCTA::BuyMenuItemInserter(PIItem const & item)
{
	...
	if (item->IsInvalid() || dynamic_cast<CWeaponKnife*>(&item->object()) )
		return;

	CArtefact* pArtefact = dynamic_cast<CArtefact*>(item);
	if (pArtefact)
		return;
	...
	CWeaponAmmo* pAmmo = dynamic_cast<CWeaponAmmo*>(item);
