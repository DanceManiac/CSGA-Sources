...
void CCharacterPhysicsSupport::in_NetSpawn( CSE_Abstract* e )
{
	...
	if( m_EntityAlife.use_simplified_visual	( ) )
	{
		...
		IKinematics*	ka = dynamic_cast<IKinematics*>( m_EntityAlife.Visual( ) );
		VERIFY( ka );
	...
	IKinematicsAnimated*ka= dynamic_cast<IKinematicsAnimated*>( pVisual );
	IKinematics*pK= dynamic_cast<IKinematics*>( pVisual );
	VERIFY( &e->spawn_ini() );
	...
	if( Type( ) == etStalker )
	{
		m_hit_animations.SetupHitMotions( *dynamic_cast<IKinematicsAnimated*>( m_EntityAlife.Visual( ) ) );
	}
	anim_mov_state.init( );
...
IC		void	CCharacterPhysicsSupport::						UpdateDeathAnims				()
{
	...
	{
		DestroyIKController( );
		dynamic_cast<IKinematicsAnimated*>(m_EntityAlife.Visual())->PlayCycle("death_init");
		m_flags.set(fl_death_anim_on,TRUE);
	}
}
#ifdef DEBUG
...
/*
		IKinematicsAnimated	*K = dynamic_cast<IKinematicsAnimated*>(m_EntityAlife.Visual());
...
void CCharacterPhysicsSupport::CreateSkeleton(CPhysicsShell* &pShell)
{
	...
	IKinematics* k = dynamic_cast<IKinematics*>(m_EntityAlife.Visual());
	...
	disable_params.Load(dynamic_cast<IKinematics*>(m_EntityAlife.Visual())->LL_UserData());
...
bool CCharacterPhysicsSupport::DoCharacterShellCollide()
{
	if(m_eType==etStalker)
	{
		CAI_Stalker*	OBJ=dynamic_cast<CAI_Stalker*>(&m_EntityAlife);
		VERIFY			(OBJ);
		return			!OBJ->wounded();
	}
	return true;
...
void	CCharacterPhysicsSupport::	AddActiveWeaponCollision		()
{
	...
	CInventoryOwner*inv_owner = dynamic_cast<CInventoryOwner*>( &m_EntityAlife );
	VERIFY( inv_owner );
...
void CCharacterPhysicsSupport::in_ChangeVisual()
{
	...
	destroy_animation_collision();
	destroy( m_interactive_motion );
	IKinematicsAnimated* KA = dynamic_cast<IKinematicsAnimated*>( m_EntityAlife.Visual( ) );
	if( KA )
	{
