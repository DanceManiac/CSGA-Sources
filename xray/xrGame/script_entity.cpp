...
{
	// sounds
	CScriptEntity	*l_tpScriptMonster = dynamic_cast<CScriptEntity*>((CGameObject*)(tpKinematics->GetUpdateCallbackParam()));
...
const Fmatrix CScriptEntity::GetUpdatedMatrix(shared_str caBoneName, const Fvector &tPositionOffset, const Fvector &tAngleOffset)
{
	...
	if (xr_strlen(caBoneName)) {
		CBoneInstance	&	l_tBoneInstance = dynamic_cast<IKinematics*>(object().Visual())->LL_GetBoneInstance(dynamic_cast<IKinematics*>(object().Visual())->LL_BoneID(caBoneName));
