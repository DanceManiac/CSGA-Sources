...
//	using CUISimpleWindow::Init;
	...
	IC		bool			IsShown				()												{return this->GetVisible();}
			...
			void			SetShowGameDifficulty(EDifficultyShowCondition max_allowed_difficulty){ showConditionGameDifficulty_ = max_allowed_difficulty; };

	IC		bool			GameDifficultyAllows()
			{
				if (showConditionGameDifficulty_ != CondNotSet)
				{
					if (showConditionGameDifficulty_ == CondNever)
						return false;
					else
						return(g_SingleGameDifficulty < showConditionGameDifficulty_);
				}
				else
					return true;
			};
...
protected:
	...
	EDifficultyShowCondition showConditionGameDifficulty_;
