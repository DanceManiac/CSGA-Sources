...
_value_type CStalkerPropertyEvaluatorEnemySeeMe::evaluate	()
{
	...
	const CCustomMonster	*enemy_monster = dynamic_cast<const CCustomMonster*>(enemy);
	if (enemy_monster)
...
_value_type CStalkerPropertyEvaluatorReadyToKill::evaluate	()
{
	...
	CWeapon&		best_weapon = dynamic_cast<CWeapon&>(*m_object->best_weapon());
...
_value_type CStalkerPropertyEvaluatorSmartTerrainTask::evaluate	()
{
	...
	CSE_ALifeHumanAbstract		*stalker = dynamic_cast<CSE_ALifeHumanAbstract*>(ai().alife().objects().object(m_object->ID(), true));
	if (!stalker)
		return					(false);
...
_value_type CStalkerPropertyEvaluatorEnemyCriticallyWounded::evaluate	()
{
	...
	const CAI_Stalker			*enemy_stalker = dynamic_cast<const CAI_Stalker*>(enemy);
	if (!enemy_stalker)
		return					(false);
