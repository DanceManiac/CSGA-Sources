...
bool CScriptGameObject::CheckObjectVisibility(const CScriptGameObject *tpLuaGameObject)
{
	...
	CScriptEntity		*script_entity = dynamic_cast<CScriptEntity*>(&object());
	if (!script_entity) {
...
Fvector	CScriptGameObject::bone_position	(LPCSTR bone_name) const
{
	...
	if (xr_strlen(bone_name))
		bone_id			= dynamic_cast<IKinematics*>(object().Visual())->LL_BoneID(bone_name);
	else
		bone_id			= dynamic_cast<IKinematics*>(object().Visual())->LL_GetBoneRoot();

	Fmatrix				matrix;
	matrix.mul_43		(object().XFORM(),dynamic_cast<IKinematics*>(object().Visual())->LL_GetBoneInstance(bone_id).mTransform);
	return				(matrix.c);
}
