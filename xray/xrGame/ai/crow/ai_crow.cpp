...
BOOL CAI_Crow::net_Spawn		(CSE_Abstract* DC)
{
	...
	// animations
	IKinematicsAnimated*	M		= dynamic_cast<IKinematicsAnimated*>(Visual()); R_ASSERT(M);
...
{
	dynamic_cast<IKinematicsAnimated*>(Visual())->PlayCycle	(m_Anims.m_fly.GetRandom());
}
void CAI_Crow::switch2_FlyIdle()
{
	dynamic_cast<IKinematicsAnimated*>(Visual())->PlayCycle	(m_Anims.m_idle.GetRandom());
}
...
//		if (first_time)	Die			(who);
	...
	else dynamic_cast<IKinematicsAnimated*>(Visual())->PlayCycle(m_Anims.m_death_dead.GetRandom());
}
//---------------------------------------------------------------------
...
void CAI_Crow::CreateSkeleton()
{
	...
	m_pPhysicsShell->SetMaterial(dynamic_cast<IKinematics*>(Visual())->LL_GetData(dynamic_cast<IKinematics*>(Visual())->LL_GetBoneRoot()).game_mtl_idx);
}
