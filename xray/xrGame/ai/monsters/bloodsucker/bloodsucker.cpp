...
void CAI_Bloodsucker::vfAssignBones()
{
	...
	bone_spine =	&dynamic_cast<IKinematics*>(Visual())->LL_GetBoneInstance(dynamic_cast<IKinematics*>(Visual())->LL_BoneID("bip01_spine"));
	bone_head =		&dynamic_cast<IKinematics*>(Visual())->LL_GetBoneInstance(dynamic_cast<IKinematics*>(Visual())->LL_BoneID("bip01_head"));
...
void CAI_Bloodsucker::start_drag()
{
	if(m_animated){
		...
		dynamic_cast<IKinematicsAnimated*>(Visual())->PlayCycle("boloto_attack_link_bone",TRUE,animation_end_jump,this);
		m_animated = false;
	}
}
