...
void game_cl_Deathmatch::OnBuyMenu_Ok	()
{
	...
	if(!l_pPlayer) return;
	CActor* pActor = dynamic_cast<CActor*>(l_pObj);
...
void game_cl_Deathmatch::SetBuyMenuItems		(PRESET_ITEMS* pItems, BOOL OnlyPreset)
{
	...
	if (pCurActor)
	{
		...
		for ( ; ISlot != ESlot; ++ISlot) 
		{
			...
			if ( pItem->IsInvalid() || dynamic_cast<CWeaponKnife*>( &pItem->object() ) )	continue;
			if (!pItem->CanTrade())
			{
				continue;
			}
...
bool game_cl_Deathmatch::LocalPlayerCanBuyItem(shared_str const & name_sect)
{
	if (name_sect == "mp_wpn_knife")
		return true;
	CUIMpTradeWnd* buy_menu = dynamic_cast<CUIMpTradeWnd*>(pCurBuyMenu);
	R_ASSERT(buy_menu);
